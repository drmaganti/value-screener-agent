...
def _final_prompt(metrics: MetricSnapshot, scores: CategoryScores, debate: dict[str, Any] | None = None) -> str:
    debate_text = json.dumps(debate, separators=(",", ":")) if debate else "none"
...
Independent debate evidence: {debate_text}
...
async def build_deep_analysis(metrics: MetricSnapshot, scores: CategoryScores, depth: str) -> tuple[DeepAnalysis, str]:
    ...
    if depth == "debate":
        ...
        bull, bear, risk = await asyncio.gather(*[_generate(p) for p in prompts.values()])
        debate = {
