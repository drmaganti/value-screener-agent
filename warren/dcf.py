...
def _discount_rate(metrics: MetricSnapshot) -> DiscountRateDetails:
    basis = [
        "Risk-free rate, equity risk premium, pre-tax debt cost and tax rate are configured methodology assumptions."
    ]
    beta = metrics.beta if metrics.beta is not None and 0.25 <= metrics.beta <= 3 else 1.0
    if beta == 1.0 and metrics.beta != 1.0:
        basis.append("Beta was missing or outside the supported range; 1.0 was used.")
    else:
        basis.append("Beta came from the Yahoo Finance company snapshot.")
    ...
    return DiscountRateDetails(
        method="CAPM cost of equity plus after-tax debt cost",
...
def calculate_dcf(metrics: MetricSnapshot) -> DcfResult:
    ...
    scenario_inputs = zip(
        ("bear", "base", "bull"),
        growth_rates,
        (min(MAX_WACC, base_discount_rate + 0.01), base_discount_rate, max(MIN_WACC, base_discount_rate - 0.01)),
        (0.02, 0.025, 0.03),
    )
    ...
    return DcfResult(
        ...
        discount_rate_details=discount_details,
        net_cash=net_cash,
        shares_outstanding=metrics.shares_outstanding,
        ...
        caveats=[
            ...
            growth_basis + ".",
            "Discount rate uses a company-specific beta and capital structure with configured market assumptions.",
